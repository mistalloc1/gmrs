  ...
  (list [{:name "John Smith" :country "USA" :checkin-until "22:00"
        ...
        [{:name "Erik Andersson" :country "Sweden" :checkin-until "24:00"
         ...
         {:name "Anna Lindqvist" :country "Sweden" :checkin-until "22:00"
          :avg-price 140 :amenities "breakfast|pool|wifi|babysitting" :age 38
          :travel-purpose "leisure"}]))
...
(deftest test-diagnose-columns-from-source
  (is (= {:name '(:tags) :country '(:tags) :checkin-until '(:tags)
         ...
         (diagnose-columns-from-source hotel-governor hotel-option-gives))
      "options")
  (is (= {:name '(:tags) :country '(:tags) :checkin-until '(:tags)
          :amenities '(:tags) :travel-purpose '(:tags) :avg-price '(:num)
           :age '(:num)}
         (diagnose-columns-from-source hotel-governor hotel-case-gives))
      "cases")
  (is (= ; TODO: {}? check case where it's important in integration
         []
         (diagnose-columns-from-source hotel-governor hotel-inter-gives))
      "inters (empty)"))
...
(deftest test-update-columns-diagnostics
  (is (= { :recs-amount 2
           ...
           {:name '(:tags) :country '(:tags) :checkin-until '(:tags)
            ...
            :age '(:num)}
           :inter-columns [] }
         (update-columns-diagnostics hotel-governor hotel-option-gives
                                     hotel-case-gives hotel-inter-gives))))

(deftest test-choose-and-prepare-mill
  (is (some #{(:mill (choose-and-prepare-mill
                     (update-columns-diagnostics
                       hotel-governor hotel-option-gives
                       hotel-case-gives hotel-inter-gives)))}
            (keys *EnabledMills*))
